...
export function NewTraceDocumentClient() {
  ...
  const submit = async () => {
    ...
    const res = await fetch("/api/documents", {
      ...
      body: JSON.stringify({ title, content, repoSlug: slug }),
    });
    setLoading(false);
    if (res.ok) {
      const doc = await res.json();
      router.push(`/u/${username}/trace/${slug}/${doc.slug}/edit`);
    } else {
  ...
  return (
    <EditorChrome
      backHref={`/u/${username}/trace/${slug}`}
